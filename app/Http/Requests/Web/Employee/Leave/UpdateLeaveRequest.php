...
class UpdateLeaveRequest extends FormRequest
{
    ...
    {
        return true; // Opsional: tambahkan pengecekan kepemilikan jika perlu
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Ambil ID leave yang sedang di-update agar tidak bentrok dengan dirinya sendiri
            $leaveId = $this->route('leave')?->id;

            // Cek tabrakan tanggal
            $overlappingLeave = LeaveApplication::where('user_id', Auth::user()->id)
                ->where('status', '!=', 'rejected')
                ->where('id', '!=', $leaveId) // Kecualikan leave yang sedang diupdate
